...
def add_emp(request):
    if request.method == 'POST':
        ...
        return HttpResponse("An Exception Occured and Employee couldn't been added successfully")
...
def filter_emp(request):
    if request.method == 'POST':
        ...
        emps = Employee.objects.all()
        if name:
            # icontains is used for ignoring the case sensitivity entered from the user
            emps = emps.filter(Q(first_name__icontains = name) | Q(last_name__icontains = name))
            # Q is used for using and and or operation
        if dept:
